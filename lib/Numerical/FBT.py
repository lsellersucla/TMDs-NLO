...
#        hu = minimize_scalar(h, bracket=None, bounds=(Q/10,10*Q), args=(), method='brent', tol=0.01, options=None).x/zero1*np.pi
        ...
        hu = minimize_scalar(h, bracket=None, bounds=(Q/10,10*Q), args=(), method='bounded', tol=0.01, options=None).x/zero1*np.pi
        if hu>2.:
            hu = 2.
        return hu
    ...
    def fbt(self,g,q,N=10,Q=10.,option=0):
        ...
        """ option: int optional:                                                     """
        """    0 transformed Ogata with optimized h                                   """
        """    1 untransformed Ogata with optimized h                                 """
